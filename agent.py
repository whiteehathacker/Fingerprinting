...
def module_crtsh(target, outdir: Path):
    """Consulta crt.sh para certificados relacionados con target."""
    ...
    if code == 200:
        try:
            ...
            safe_print(f"[crt.sh] error parseando JSON: {e}")
            save_json(outdir / "crtsh_raw.json", {"code": code, "text": text})
    else:
        safe_print(f"[crt.sh] error status={code}")
...
def menu_loop():
    ...
    if not target:
        ...
        return
    outdir = Path("output") / target.replace("://", "").replace("/", "_")
    ...
    # pedir SerpAPI key opcional
    ans = prompt_input("¿Tienes SerpAPI key y quieres usarla para dorks? (s/N): ").strip().lower()
    if ans == "s" or ans == "si":
        cfg["serpapi_key"] = prompt_input("Introduce SerpAPI key: ").strip()
    # permitir o no fallback scraping
    ans = prompt_input("¿Permitir fallback a scraping si SerpAPI falla o no se proporciona? (S/n): ").strip().lower()
    cfg["allow_scrape_fallback"] = (ans != "n" and ans != "no")
    # github token opcional
    ans = prompt_input("¿Tienes GitHub token para mejorar búsquedas? (s/N): ").strip().lower()
    if ans in ("s", "si"):
        cfg["github_token"] = prompt_input("Introduce GitHub token: ").strip()
    # permitir añadir dorks extra
    ans = prompt_input("¿Quieres añadir dorks extra ahora? (s/N): ").strip().lower()
    if ans in ("s", "si"):
    ...
    # Menu principal
    while True:
        safe_print("\nMenú principal — objetivo:", target)
        safe_print("[1] Ejecutar enumeración pasiva (crt.sh, Wayback, DNS, GitHub, Dorks, robots/sitemap)")
        safe_print("[2] Ejecutar enumeración activa (WHOIS, nmap, whatweb, sublist3r si instalados)")
        safe_print("[3] Ejecutar solo Google Dorks")
        safe_print("[4] Añadir/editar dorks extra")
        safe_print("[5] Mostrar estado de herramientas instaladas")
        safe_print("[6] Salir")
        choice = prompt_input("Elige una opción [1-6]: ").strip()
        if choice == "1":
            ...
            safe_print("Dorks actuales adicionales:", cfg.get("extra_dorks"))
            safe_print("Introduce nuevas dorks (una por línea). Línea vacía para terminar.")
            new = []
            while True:
                l = prompt_input("> ").strip()
                if not l:
                    break
                new.append(l)
            cfg["extra_dorks"].extend(new)
            save_json(outdir / "extra_dorks.json", cfg["extra_dorks"])
            safe_print(f"Ahora tienes {len(cfg['extra_dorks'])} dorks adicionales.")
        elif choice == "5":
            tools = ["nmap", "whatweb", "sublist3r", "gobuster", "host", "nslookup", "whois"]
            status = {t: is_installed(t) for t in tools}
            safe_print("Estado herramientas instaladas:")
            for t, v in status.items():
                safe_print(f"  {t}: {'YES' if v else 'NO'}")
        elif choice == "6":
            safe_print("Saliendo. Resultados en:", outdir)
            break
        else:
